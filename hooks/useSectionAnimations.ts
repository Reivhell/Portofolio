...
function initAnimations() {
    ...
    /* 2. SERVICES — mouse-follow 3D tilt (desktop only) ─── */
    if (!isMobile) {
    ...
    /* 3. STACK — floating hover effect on chips (desktop only) ─── */
    if (!isMobile) {
    ...
    /* 6. SKILLS — hover scale on list items (desktop only) ─── */
    if (!isMobile) {
    ...
    /* 8. TESTIMONIALS — 3D tilt mouse follow (desktop only) ─── */
    if (!isMobile) {
    ...
    /* 9. FOOTER — magnetic social button hover (desktop only) ─── */
    if (!isMobile) {
    ...
    /* 10. SPLITTEXT — per-char reveal on section titles (desktop only) ─── */
    gsap.utils
